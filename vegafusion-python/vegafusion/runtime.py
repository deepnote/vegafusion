...
class VegaFusionRuntime:
    ...
    def pre_transform_logical_plan(
        ...
        dict[str, Any], list[dict[str, Any]], list[PreTransformWarning]
    ]:
        """
        Evaluate supported transforms in an input Vega specification and return
        logical plans for the transformed datasets.

        Produces a new specification with small pre-transformed datasets included
        inline and logical plans for larger datasets.
        ...
        Returns:
            ...
            * The Vega specification as a dict with pre-transformed datasets
              included but left empty.
            * Logical plans as a list of dictionaries, each containing:
               * ``name``: dataset name
               * ``scope``: dataset scope list
               * ``logical_plan``: string representation of the logical plan
               * ``sql``: SQL representation of the logical plan
            * A list of warnings as dictionaries. Each warning dict has a ``'type'``
              key indicating the warning type, and a ``'message'`` key containing
              a description of the warning. Potential warning types include:
        ...
        new_spec, logical_plans, warnings = self.runtime.pre_transform_logical_plan(
            spec,
            local_tz=local_tz,
            default_input_tz=default_input_tz,
            preserve_interactivity=preserve_interactivity,
        ...
        return new_spec, logical_plans, warnings
