...
def _discover_spec_files(limit: int | None = None) -> list[Path]:
...
def spark():
    ...
    sales_data_df = sales_data_df.withColumn("datetime", (col("datetime") / 1e9).cast("timestamp"))
...
def test_spec_against_spark(spec_path: Path, spark: SparkSession):
    ...
    spark_dataframes = {
        ds["name"]: spark.sql(ds["sparksql"]).toPandas() if "sparksql" in ds else ds["data"].to_pandas()
        for ds in spark_datasets_sql if ds["namespace"] == "data"
    }
    
    print('Inmemory datasets:', set(inmemory_dataframes.keys()))
    print('Spark datasets:', set(spark_dataframes.keys()))
    assert set(inmemory_dataframes.keys()) == set(spark_dataframes.keys()), "Mismatch in returned dataframes"
