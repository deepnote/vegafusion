...
def test_pre_transform_logical_plan_vendor_has_sparksql():
    ...
    for update in export_updates:
        ...
        if "sparksql" in update:
            found_sparksql = True
            assert isinstance(update["sparksql"], str), f"sparksql should be string, got {type(update['sparksql'])}"
            assert len(update["sparksql"]) > 0, "sparksql should not be empty"
            print(f"✓ Found sparksql for dataset '{update['name']}': {update['sparksql']}")
